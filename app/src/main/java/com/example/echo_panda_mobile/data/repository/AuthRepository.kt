...
class AuthRepository(
    ...
    suspend fun login(request: LoginRequest): AuthResult<AuthResponse> {
        ...
        // Fresh start: Clear everything before attempting login
        logoutLocally()
    ...
    suspend fun storeNewPasswordInFirestore(email: String, newPassword: String): AuthResult<Unit> {
        return try {
            ...
            if (query.isEmpty) {
                return AuthResult.Error("User not found in Firestore.")
            }

            val docId = query.documents[0].id
            // Note: Storing raw passwords in Firestore is generally not recommended for production.
            // This is implemented as requested by the ViewModel logic.
    ...
    private suspend fun syncBackendSession(
        ...
        return try {
            ...
            // After successful session creation, clear any cached interceptors
            // to ensure they pick up the fresh token from TokenStorage.
            RetrofitClient.clearInstances()
